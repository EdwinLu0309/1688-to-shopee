...
## 配號守門員：既有列規格空白就擋（`SpecGapBlocked`，2026-09-18）
配號是拿 **1688 規格原文**（SKU表 L/M）比對既有列來決定「沿用舊碼還是發新碼」。那格空白就比不到 →
**已經有品號的選項會被當成新的、再發一個品號**；同一個選項兩個碼，蝦皮算一邊、成本算另一邊，獲利表對不起來且不報錯。
- 現行＝`plan_blocks`／`write_staging` 前先跑 `existing_spec_gaps`：**這次要做的商品編號**在 1-1 既有列有任一 L 空白 → 丟 `SpecGapBlocked`，
  GUI 跳對話框列出品號並說明三種處理（補規格／標停售退出訂貨表／這次不做這支）。全新編號查不到既有列＝不擋。
- ⚠️ 只看這次要做的編號，不看整張表——整張表的空白多半是早就停售的舊資料，一律擋會變成永遠跑不動。
- Lady 2026-09-18 實況：2,211 列中 1,082 列規格空白，但**有標籤（真的在訂貨表跑）的只剩 20 列**，全是
  「內衣組合附的內褲」與「NG 瑕疵品」——不去 1688 下單，空白是合理的。另 12 列（H-c40／O-b11／H-c10 黑S）Edwin 已標停售退出。
- 回歸測試 `tests/test_spec_gap_guard.py`（8 項）。
...
- ⚠️ **1-1「📦 新品」選單（三家 `*_master_Code.gs`）原本只貼 A~D／F~H／L~M，不帶 J** → 程式寫的 #NEW 到不了 SKU 表。
  2026-09-17 `NP_SKU_RANGES` 加 `[10, 1]`，**要 Edwin 貼回三家 Apps Script 才生效**（三家 SKU表 J 都是死值欄、非公式，已驗）。
...
- **三家名單同一個版面（2026-09-19 Lady 比照 Nail 重排）**：第 1 列 `匯率｜IMPORTRANGE(1-1 設定!B2)｜蝦皮折扣｜折扣值`；
  第 2 列表頭；**第 3 列＝說明列**（編號欄以 `※` 開頭，程式跳過）；第 4 列起資料。
  順序＝`訂貨需求｜編號｜商品 or 品牌名稱｜分類｜(子分類)｜標籤｜安全存量｜進貨網址｜選項說明｜(尺寸)｜商品成本｜台幣成本｜最低｜最高｜最後定價｜最後毛利｜蝦皮設定售價`，
  括號＝Lady 才有。寫「自動」的五欄是**說明列那格的陣列公式**（`={"說明";ARRAYFORMULA(...)}`）：上面顯示說明、第 4 列起溢出算好的值，
  刪資料列不會弄掉公式。Lady 的研究欄（填表日期／參考網址／月銷／對手售價…）已移出（研究放「新品研究」表）；舊表備份在 `data/backups/ai_lists_backup_0919.json`。
  ⚠️ Lady 的 B1 IMPORTRANGE 第一次要**人**在表上按「允許存取」，沒按之前台幣成本那幾欄是 `#REF!`（掛牌價不受影響）。
- ⚠️⚠️ **「蝦皮設定售價」空白不可退回抓最後一個數字**（2026-09-19）：Nail 那五欄「自動」其實沒有公式，
  舊版就抓到「最後定價」（折後價）當掛牌價 → LTL142 掛 69（應為 99），蝦皮再打 7 折＝賣便宜 30% 且不報錯。
  現行＝那欄空就用 `最後定價 ÷ 表頂折扣` 自己算並 warning；兩個都空回 0。`tests/test_list_price.py`（6 項）。
...
## Lady 上架規則（2026-09-18 接上，SOP＝`config/sop/lady/JoysLu_Lady_上架SOP_v1.0.md`）
- **標題**＝ 9/17 A/B 的 A 版：`[最大詞][第二大詞][這支的真實形態詞] [依量補到 58-60]`；
  **不放【JoysLu Lady】、不放品號、零符號（含 emoji）、不寫免運/隔日/現貨/預購**。舊的 03f＋v2.4（Hook 三行＋8 區塊）退成查閱用。
- **詳情**＝中間版：AI 寫 商品特色／商品規格／穿搭與洗滌；款式說明程式列；注意事項固定文案（服飾／襪類）。
  ⚠️ 尺寸只寫程式給的尺碼標籤（公斤範圍），**不讓 AI 編胸圍腰圍**。
- ⚠️⚠️ **同一類的詞混著不同款式**（女裝-褲 同時有 工裝褲／吊帶褲／西裝褲／連身褲）→ 湊字數會寫出不實標示。
  2026-09-18 首跑實際發生（亞麻闊腿褲被補上工裝褲、吊帶褲、西裝褲、連身褲）。現行＝prompt 明令只放「這支商品本身就是」的詞、
  **寧可短也不硬湊**；`check_title(pool=…)` 另外把「不在搜尋詞庫的詞」列進核對表提醒（形態詞除外）。
- **字數湊不滿的順序（Edwin 2026-09-18）**：①詞庫的詞（搜尋量優先）②用完還不到 58 字 → **從這支商品自己的資料補特徵詞**
  （亞麻／高腰／垂感／薄款／無彈，要在屬性或詳情找得到，一個詞一格不要串成一長串）③還是不足就留短。
  `check_title` 只對「詞庫沒有、商品資料也找不到」的詞提醒（串成一長串的會拆兩字比對）。實測 P14AE1 亞麻闊腿褲 43 字、0 個假詞。
- 搜尋詞庫分賣場（`POOLS`）：Nail 相關性取「美甲」、有廣域、化粧品要 ✅PIF合規；
  **Lady 取「內著」＋「預購」**（自動上架做的正是預購女裝）、**沒有廣域**（內褲的標題不該出現絲襪的詞）、位階「錯字」也排除。
- Lady AI 上架名單 2026-09-18 補 `V 子分類｜W 標籤｜X 安全存量`，`U 蝦皮設定售價` 改公式＝`最後定價 ÷ F1 折扣`（F1=0.5）。
...
- ⚠️ **選項超過 12 個就壓縮**（服飾動輒 4 色 × 6 尺碼 ＝ 72 個，一格塞 72 行沒人看得完）：
  選項名稱寫「72 個選項＝12 × 6」＋第一軸清單＋尺碼一行；售價同價寫一個、不同寫區間；品號只列前 3 個＋總數。
- Lady 核對表＝`1yTkrDfKsqmWvk…`（與【Lady】AI上架名單同資料夾，2026-09-18 建、已分享 SA）。
