#!/bin/bash
# 1688 訂單刷新（金流核對）（Mac 啟動）— 獨立金流核對 GUI（reconcile_gui.py），與 gui.py / order_gui.py 分開
cd "$(dirname "$0")"
...
if [ -x ".venv/bin/python" ]; then
    ...
    exec .venv/bin/python reconcile_gui.py
fi
