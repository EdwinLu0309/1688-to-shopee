/**
 * 【Lady】商品主檔 — 綁定 Apps Script（選單 4 按鈕 + onEdit 自動記錄）
 *
 * 2026-08-07 比照 Nail 最新版全面升級（nail_master_Code.gs 571b5ea）：
 ...
 *
 * 安裝：Lady 主檔 → 擴充功能 → Apps Script → 貼上取代全部 → 儲存 → 重整試算表，
...
function snapshotToAmountRecord(silent) {
  var ui = SpreadsheetApp.getUi(), ss = SpreadsheetApp.getActiveSpreadsheet();
  if (!AMOUNT_SHEET_ID) { if (silent) throw new Error("未設定 AMOUNT_SHEET_ID"); ui.alert("尚未設定 Lady 進貨金額記錄表 ID"); return; }
...
//   SKU表 ：A品號 B品名 C分類 D標籤 ┊ F成本 G幣別 H安全存量 ┊ J選項註記(#NEW 日期) ┊ L規格一 M規格二
// 連續的合併成一個 range 一次寫（少幾次 API，也不會誤觸公式欄）
var NP_PROD_RANGES = [[1, 5], [7, 1], [10, 3]];     // [起始欄, 欄數]：A~E / G / J~L
var NP_SKU_RANGES  = [[1, 4], [6, 3], [10, 1], [12, 2]];  // A~D / F~H / J / L~M（J＝新品記號 #NEW MMDD，2026-09-17）
