/**
 * 【Baby】蝦皮上架協作檔 — 商品清單同步（獨立排程版；自 upload_collab_sync.gs Nail 版移植）
 *
 * 用途：把「【Baby】商品主表(1-1)」的商品清單（商品編號/分類/品名）定時同步進「共用雲的
 ...
 *   1) 打開 Baby 協作檔 → 擴充功能 → Apps Script → 貼上本檔 → 儲存。
 *   2) 編輯器上方選 setupCollab → 執行 → 跳授權請同意
 *      （自動：分頁改名/建「蝦皮狀態」＋寫 12 欄表頭＋首輪灌入商品清單）。
 *   3) 回協作檔確認清單被填好（~280 商品）。
 *   4) 選 installSchedule → 執行 → 裝好「每 30 分鐘」排程（重複執行會先清舊觸發器，不疊加）。
 */

var MASTER_ID  = '1yIrFP_lJZ1Tq6VTIi_6J7xC6NZtTYXexwsoJyF3CNpo';  // 【Baby】商品主表(1-1)
var MASTER_TAB = '商品表';                                        // 商品識別來源分頁
var COLLAB_ID  = '1Y_l2HuygnN7AkjTAqeLDoLdjOf7n6tAOAAwEi3bqG1I';   // 【Baby】員工上架協作檔
