/**
 * 【Lady】2-1 進貨金額記錄 — 綁定 Apps Script（2-1 自己的功能）
 * 安裝：開 2-1 → 擴充功能 → Apps Script → 貼上 → 儲存 → 重整，出現「🚀 金額核對」選單。
 * 責任分界：1-1 到「產出 2-1 金額分頁」為止；2-1 核對完由本腳本傳到 2-2 做收尾。
 *
 * 2026-08-06 自 nail_amount_transfer_Code.gs 移植（含「一廠商多訂單」修正）：
 *   2-1 的 B 付款平台訂單編號是 TEXTJOIN 出來的多筆字串（"O1, O2, O3"），本腳本
 *   「拆開每個訂單編號、各自一列、各自 XLOOKUP 運送單號」，到 2-2 才抓得到每筆訂單
 *   的運送單號。每組列數 = max(商品數, 訂單數)：商品(A/B)、訂單(D)/運送單號(E)各自
 *   逐列排，C 廠商整組合併。
 */
var TZ = "Asia/Taipei";
var ARRIVAL_SHEET_ID = "1befHjwN434vLtjJplqGSxOBT-7D6IVJqfgZUK1N1Rww"; // 【Lady】2-2 商品到貨記錄
