...
class ShopProfile:
    ...
    # 詳情：空字串＝沿用 prompt 預設（Lady 8 區塊）；有值＝AI 只寫這裡講的段落，
    # 款式說明與注意事項由 scraper/detail_builder.py 程式組（Nail 2026-09-17）
    detail_rule: str = ""
    # 標題：空字串＝沿用 prompt 預設（品牌標籤＋核心關鍵字＋編號）；有值＝整段取代（Nail v2.2）
    title_rule: str = ""
    title_version: str = ""       # 標題規則版號；和快取文案不同 → 快取作廢重生
    detail_version: str = ""      # 組裝規則版號；和快取文案不同 → 快取作廢重生
...
_LADY = ShopProfile(
    ...
    sop_files=["lady/JoysLu_Lady_上架SOP_v1.0.md"],   # 舊的 03f＋v2.4 仍在 config/sop/ 供查閱與當模板選項
    ...
    # 標題＝2026-09-17 A/B 的 A 版公式（Edwin 9/18 定：拿掉品牌標籤、符號、免運前綴與品號）
    title_rule=(
        "依 SOP 標題規範：**不放品牌標籤【JoysLu Lady】、不放商品編號、不放 🏆!【】等符號、"
        "不寫免運／隔日到貨／現貨／預購**；最前面放下方【第一行大詞候選】裡真正描述這支商品的 1~2 個大詞，"
        "接著放這支商品自己的真實形態詞（冰絲丁字褲／分段壓力長筒襪），"
        "其餘依搜尋量降冪補到 58-60 字；只用清單裡的詞，不寫商品沒有的屬性"),
    title_version="lady_title_v1",
    detail_rule=(
        "詳情只寫下面三段（每段標題用「✦ 」開頭），其餘段落程式會另外補，**不要寫**："
        "款式／顏色、注意事項、賣場介紹、退換貨、推薦搭配、Hook 三行標題。\n"
        "   ✦ 商品特色：2-3 行，只根據商品名與屬性表（材質、版型、機能），不誇大\n"
        "   ✦ 商品規格：只列屬性表或商品名裡明確寫到的（材質成分、厚薄、彈力、版型）；"
        "**尺寸只寫程式給你的尺碼標籤（S/M/L 與公斤範圍），不要自己編胸圍腰圍數字**；沒資料就整段省略，"
        "絕不寫「待補」「詳見商品標示」\n"
        "   ✦ 穿搭與洗滌：3-5 行，怎麼搭、洗滌注意（依品類寫通用做法，不編廠商沒寫的機能）"),
    detail_version="lady_detail_v1",
)
...
_NAIL = ShopProfile(
    ...
    detail_rule=(
        "詳情只寫下面三段（每段標題用「✦ 」開頭），其餘段落程式會另外補，**不要寫**："
        "款式／顏色、注意事項、賣場介紹、退換貨、推薦搭配。\n"
        "   ✦ 商品特色：2-3 行，只根據商品名與屬性表，不誇大、不寫沒有根據的機能\n"
        "   ✦ 商品規格：只列屬性表或商品名裡明確寫到、且屬於這次上架款式的項目（材質、容量、功率、尺寸…）；"
        "沒有就**整段省略**，絕不寫「待補」「詳見商品標示」，也不要寫同頁其他款的規格\n"
        "   ✦ 使用方法：依品類寫 3-5 步通用步驟（膠類：修甲→底膠→色膠→封層照燈；器材：操作與清潔保養；"
        "耗材：怎麼更換）"),
    detail_version="nail_detail_v1",
    title_rule=(
        "依 SOP §2 標題規範 v2.2：**不放品牌標籤【JoysLu Nail】、不放商品編號、除 ✅ 外零符號**；"
        "最前面放下方【第一行大詞候選】裡真正描述這支商品的 1~2 個大詞，接著放這支商品自己的真實形態詞；"
        "化粧品（有【這支要放 ✅PIF合規】那行）接 ✅PIF合規 再接品牌；器材接型號與已驗證規格；"
        "尾串只用搜尋詞庫的詞，填到 58-60 字"),
    title_version="nail_title_v2.2",
)
