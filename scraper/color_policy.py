...
# 拆底色時把它們剝掉，只留純顏色來判斷保留與否。Claude 繁體渲染很多變體
# （常規款/常規版/嬌小版/高挑版…），故盡量列全；但底色分組用「簡體原始 key」為準最穩。
_VARIANT_TOKENS = [
    "常规款", "常規款", "高个子", "高個子", "小个子", "小個子",
    "常规版", "常規版", "标准款", "標準款", "标准版", "標準版",
    "嬌小版", "娇小版", "嬌小", "娇小", "高挑版", "高挑", "矮个", "矮個",
    "高个版", "高個版", "小个版", "小個版", "大码版", "大碼版", "加大",
    "加绒", "加絨", "加厚", "薄款", "厚款", "常规", "常規", "标准", "標準",
    "升级面料", "升級面料", "版",
]
...
def select_first_axis(tier1_keys: list[str], color_map: dict, n_sizes: int,
    ...
    # 底色分組用「簡體原始 key」為準（常规款/高个子 穩定），不用 Claude 繁體值（渲染多變）。
    key_base = {k: (base_color(k) or k) for k in tier1_keys}
